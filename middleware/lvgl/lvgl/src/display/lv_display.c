...
lv_display_t *lv_display_create(int32_t hor_res, int32_t ver_res)
{
    ...
    if (disp->layer_head == NULL)
        return NULL;
    lv_layer_init(disp->layer_head);

    if (disp->layer_init)
        disp->layer_init(disp, disp->layer_head);
...
void lv_display_delete(lv_display_t *disp)
{
    ...
    if (disp->layer_deinit)
        disp->layer_deinit(disp, disp->layer_head);
    lv_free(disp->layer_head);

    lv_free(disp);

    if (was_default)
        lv_display_set_default(lv_ll_get_head(disp_ll_p));
...
void lv_display_set_resolution(lv_display_t *disp, int32_t hor_res, int32_t ver_res)
{
    ...
    if (disp->hor_res == hor_res && disp->ver_res == ver_res)
        return;
...
void lv_display_set_color_format(lv_display_t *disp, lv_color_format_t color_format)
{
    ...
    if (disp->buf_1)
        disp->buf_1->header.cf = color_format;
    if (disp->buf_2)
        disp->buf_2->header.cf = color_format;
    if (disp->buf_3)
        disp->buf_3->header.cf = color_format;
...
lv_obj_t *lv_display_get_screen_by_name(const lv_display_t *disp, const char *screen_name)
{
    ...
    for (i = 0; i < disp->screen_cnt; i++) {
        ...
        if (n && lv_streq(screen_name, n))
            return disp->screens[i];
    }
...
void lv_screen_load_anim(lv_obj_t *new_scr, lv_screen_load_anim_t anim_type, uint32_t time, uint32_t delay, bool auto_del)
{
    lv_display_t *d = lv_obj_get_display(new_scr);
    if (!d) {
        LV_LOG_WARN("lv_screen_load_anim: new_scr has no display (new_scr=%p)", (void *)new_scr);
        return;
    }

    lv_obj_t *act_scr = d->act_scr;

    LV_LOG_USER("lv_screen_load_anim called: new_scr=%p act_scr=%p scr_to_load=%p", (void *)new_scr, (void *)act_scr, (void *)d->scr_to_load);

    if (act_scr == new_scr) {
        LV_LOG_USER("lv_screen_load_anim: early return because act_scr == new_scr");
        return;
    }
    if (d->scr_to_load == new_scr) {
        LV_LOG_USER("lv_screen_load_anim: early return because scr_to_load == new_scr");
        return;
    }
...
void lv_display_set_theme(lv_display_t *disp, lv_theme_t *th)
{
    ...
    if (disp->screen_cnt == 4 && lv_obj_get_child_count(disp->screens[0]) == 0 && lv_obj_get_child_count(disp->screens[1]) == 0 &&
...
uint32_t lv_display_get_invalidated_draw_buf_size(lv_display_t *disp, uint32_t width, uint32_t height)
{
    ...
    if (disp->buf_2)
        LV_ASSERT(disp->buf_2->data_size >= buf_size);
    if (disp->buf_3)
        LV_ASSERT(disp->buf_3->data_size >= buf_size);
...
static void scr_load_internal(lv_obj_t *scr)
{
    ...
    if (old_scr)
        lv_obj_send_event(old_scr, LV_EVENT_SCREEN_UNLOAD_START, NULL);
    ...
    if (old_scr)
        lv_obj_send_event(old_scr, LV_EVENT_SCREEN_UNLOADED, NULL);
...
{
    return anim_type == LV_SCREEN_LOAD_ANIM_FADE_OUT || anim_type == LV_SCREEN_LOAD_ANIM_OUT_LEFT || anim_type == LV_SCREEN_LOAD_ANIM_OUT_RIGHT ||
           anim_type == LV_SCREEN_LOAD_ANIM_OUT_TOP || anim_type == LV_SCREEN_LOAD_ANIM_OUT_BOTTOM;
}
