...
ATTR_RAMFUNC_WITH_ALIGNMENT(ADC_SOC_DMA_ADDR_ALIGNMENT) uint32_t pmt_buff[APP_ADC16_PMT_DMA_BUFF_LEN_IN_4BYTES];
#ifdef HPM_MONITOR_ENABLE
extern uint16_t* adc_buff1;
extern uint16_t* adc_buff2;
#else
...
void hdma_dma_chain_config(void)
{
...
#ifdef HPM_MONITOR_ENABLE
    dma_ch_config.dst_addr = core_local_mem_to_sys_address(HPM_CORE0, (uint32_t)adc_buff1);
#else
    ...
    dma_ch_config.src_addr = core_local_mem_to_sys_address(HPM_CORE0, (uint32_t)pmt_buff);
#ifdef HPM_MONITOR_ENABLE
    dma_ch_config.dst_addr = core_local_mem_to_sys_address(HPM_CORE0, (uint32_t)adc_buff2);
#else
...
#ifdef HPM_MONITOR_ENABLE
    ch_config.dst_addr = core_local_mem_to_sys_address(HPM_CORE0, (uint32_t)&adc_buff1);
#else
...
{
#ifdef HPM_MONITOR_ENABLE
    return (index == 0) ? (uint8_t *)adc_buff1 : (uint8_t *)adc_buff2;
#else
