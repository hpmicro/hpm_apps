	...
	if(full_port_tsn_get_recv_dma_irq_state()) {
		do {
			...
			if(!TSW_S2MM_RESP_DECERR_GET(u32RecvResp) && !TSW_S2MM_RESP_SLVERR_GET(u32RecvResp)) {
				...
				uint8_t u8SrcMac[6], u8DesMac[6];
				memcpy(u8DesMac, (uint8_t*)(&(recv_buff[u8Id][16])), 6);
				memcpy(u8SrcMac, (uint8_t*)(&(recv_buff[u8Id][22])), 6);
				memcpy((uint8_t*)(&(recv_buff[u8Id][16])), u8SrcMac, 6);
				memcpy((uint8_t*)(&(recv_buff[u8Id][22])), u8DesMac, 6);
				full_port_tsn_set_send((uint8_t*)(&(recv_buff[u8Id][0])), u16Len, u8Id);
				full_port_tsn_set_recv((uint8_t*)(&(recv_buff[u8Id][0])), TSN_RECV_LENGTH, u8Id);
			} else {
...
int main(void)
{
	...
	full_port_tsn_set_broadcast_frame_action(tsw_dst_port_cpu | tsw_dst_port_1 | tsw_dst_port_2 | tsw_dst_port_3);
	full_port_tsn_set_unknown_frame_action(tsw_dst_port_null);

	full_port_tsn_set_lookup_table(0, tsw_dst_port_cpu, ((uint64_t)u8CpuPortDesMac[0])<<40 | ((uint64_t)u8CpuPortDesMac[1])<<32 | ((uint64_t)u8CpuPortDesMac[2])<<24 |
	 												((uint64_t)u8CpuPortDesMac[3])<<16 | ((uint64_t)u8CpuPortDesMac[4])<< 8 | ((uint64_t)u8CpuPortDesMac[5])<< 0);
	full_port_tsn_set_lookup_table(1, tsw_dst_port_1,   ((uint64_t)u8Port1DesMac[0])<<40 | ((uint64_t)u8Port1DesMac[1])<<32 | ((uint64_t)u8Port1DesMac[2])<<24 |
	 												((uint64_t)u8Port1DesMac[3])<<16 | ((uint64_t)u8Port1DesMac[4])<< 8 | ((uint64_t)u8Port1DesMac[5])<< 0);
	full_port_tsn_set_lookup_table(2, tsw_dst_port_2,   ((uint64_t)u8Port2DesMac[0])<<40 | ((uint64_t)u8Port2DesMac[1])<<32 | ((uint64_t)u8Port2DesMac[2])<<24 |
	 												((uint64_t)u8Port2DesMac[3])<<16 | ((uint64_t)u8Port2DesMac[4])<< 8 | ((uint64_t)u8Port2DesMac[5])<< 0);
	full_port_tsn_set_lookup_table(3, tsw_dst_port_3,   ((uint64_t)u8Port3DesMac[0])<<40 | ((uint64_t)u8Port3DesMac[1])<<32 | ((uint64_t)u8Port3DesMac[2])<<24 |
	 												((uint64_t)u8Port3DesMac[3])<<16 | ((uint64_t)u8Port3DesMac[4])<< 8 | ((uint64_t)u8Port3DesMac[5])<< 0);
