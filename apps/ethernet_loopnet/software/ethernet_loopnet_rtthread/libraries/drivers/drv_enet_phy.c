...
static uint8_t phy_link_state[2] = {0};

uint8_t get_phy_link_state(uint8_t phy_index)
{
    return phy_link_state[phy_index];
}
...
static void phy_poll_status(void *parameter)
{
    ...
    for (uint32_t i = 0; i < phy_monitor_handle->phy_handle_cnt; i++)
    {
        ...
        if(phy_dev->phy.bus->hw_obj == HPM_ENET0)
        {
            phy_link_state[0] = phy_dev->phy_link;
        }
        else if(phy_dev->phy.bus->hw_obj == HPM_ENET1)
        {
            phy_link_state[1] = phy_dev->phy_link;
        }
    }
}
