...
hpm_stat_t enet_init(ENET_Type *ptr)
{
    ...
    hpm_stat_t stat;
        ...
        #elif defined(__USE_RTL8211F) && __USE_RTL8211F
        rtl8211f_config_t phy_config;
        #endif
    ...
    /* Initialize phy */
        ...
        #elif defined(__USE_RTL8211F) && __USE_RTL8211F
        rtl8211f_reset(ptr, RTL8211F_ADDR);
        rtl8211f_basic_mode_default_config(ptr, &phy_config);
        if (rtl8211f_basic_mode_init(ptr, RTL8211F_ADDR, &phy_config) == true) {
        #endif
    #elif defined(HPM_ENET_RMII) && HPM_ENET_RMII
        #if defined(__USE_DP83848) && __USE_DP83848
        ...
        jl1111_reset(ptr, JL1111_ADDR);
        jl1111_basic_mode_default_config(ptr, &phy_config);
        phy_config.media_interface = enet_inf_mii;
        if (jl1111_basic_mode_init(ptr, JL1111_ADDR, &phy_config) == true) {
    #endif
...
void enet_self_adaptive_port_speed(void)
{
    ...
    #elif defined(__USE_RTL8211F) && __USE_RTL8211F
        rtl8211f_get_phy_status(ENET, RTL8211F_ADDR, &status);
    #endif
#else
    #if defined(__USE_DP83848) && __USE_DP83848
