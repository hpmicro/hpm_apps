...
static void iperf_udp_client(void *thread_param)
{
    ...
    uint32_t *buffer = pvPortMalloc(IPERF_BUFSZ);
    if (buffer == NULL) {
...
static void iperf_client(void *thread_param)
{
    ...
    uint8_t *send_buf = (uint8_t *)pvPortMalloc(IPERF_BUFSZ);
    if (send_buf == NULL) {
        return;
    }
...
void iperf_server(void *thread_param)
{
    ...
    uint8_t *recv_data = (uint8_t *) pvPortMalloc(IPERF_BUFSZ);
    if (recv_data == NULL) {
        LOG_E("No memory!\r\n");
        goto __exit;
    }
...
int iperf(int argc, char **argv)
{
    ...
    if (param.mode == IPERF_MODE_STOP) {
        ...
        param.port = port;
        if (param.host) {
            vPortFree(param.host);
            param.host = NULL;
        }
        if (host) {
            uint32_t copy_size = strlen(host);
            char *host_buffer = pvPortMalloc(copy_size + 1);
