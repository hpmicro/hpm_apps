...
void tsw_self_adaptive_port_speed(void)
{
    ...
    jl1111_get_phy_status(BOARD_TSW, TSW_TSNPORT_PORT1, &status);
    ...
    if (status.tsw_phy_link || (status.tsw_phy_link != last_status.tsw_phy_link)) {
        if (memcmp(&last_status, &status, sizeof(tsw_phy_status_t)) != 0) {
            ...
            if (status.tsw_phy_link) {
                ...
                // tsw_set_port_speed(BOARD_TSW, TSW_TSNPORT_PORT1, port_speed[status.tsw_phy_speed]);
                // tsw_set_port_speed(BOARD_TSW, TSW_TSNPORT_PORT2, port_speed[status.tsw_phy_speed]);
