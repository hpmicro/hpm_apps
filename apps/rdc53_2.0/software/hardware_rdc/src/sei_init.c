...
uint8_t sei_config(void)
{
    ...
    sei_transceiver_config_init(BOARD_SEI, BOARD_SEI_CTRL, &tranceiver_config);
    ...
    trigger_input_conifg.trig_period_time = (0.025* (clock_get_frequency(BOARD_MOTOR_CLK_NAME) / 1000));    /* unit: 1ms, 200ms */
...
{
    sei_angleCalHdl->maxph = 8388608;
    sei_angleCalHdl->motor_pole = 1;
    sei_angleCalHdl->z = sei_get_data_value(HPM_SEI, SEI_DAT_7);
    sei_angleCalHdl->ph = sei_get_data_value(HPM_SEI, SEI_DAT_5);
    sei_angleCalHdl->pos0 = sei_angleCalHdl->ph + sei_angleCalHdl->z*sei_angleCalHdl->maxph ;
}
...
void sei_angle(sei_angle_CalObj* sei_angleCalHdl)
{
    ...
    sei_angleCalHdl->ph = sei_get_data_value(HPM_SEI, SEI_DAT_5);   
     ...
     sei_angleCalHdl->pos = sei_angleCalHdl->ph +  sei_angleCalHdl->z *sei_angleCalHdl->maxph; 
     uint32_t enc_cnt = sei_angleCalHdl->maxph/sei_angleCalHdl->motor_pole;
     int32_t delta_pos = sei_angleCalHdl->pos- sei_angleCalHdl->pos0;
     if(delta_pos>=0)
     {
         sei_angleCalHdl->elec_angle =  360-((abs(delta_pos)%enc_cnt)*360)/enc_cnt ;
     }
     else
     {
              
          sei_angleCalHdl->elec_angle =  ((abs(delta_pos)%enc_cnt)*360)/enc_cnt ;
     }
     sei_angleCalHdl->elec_angle = sei_angleCalHdl->theta0 + sei_angleCalHdl->elec_angle ;
     if(sei_angleCalHdl->elec_angle >360)
     {
      sei_angleCalHdl->elec_angle  = sei_angleCalHdl->elec_angle -360;
     }
}
