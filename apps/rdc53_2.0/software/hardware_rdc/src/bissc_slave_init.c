...
int bissc_slave_init(void)
{
    ...
    uint8_t instr_idx;
    bissc_pins_init(BOARD_SEI, BOARD_SEI_CTRL);
    ...
    data_format_config.word_len = 32;
    ...
    sei_set_instr(BOARD_SEI, instr_idx++, SEI_INSTR_OP_SEND, SEI_INSTR_S_CK_TIMEOUT_EN, SEI_DAT_5, SEI_DAT_2, 32);  /* MT */
    sei_set_instr(BOARD_SEI, instr_idx++, SEI_INSTR_OP_SEND, SEI_INSTR_S_CK_TIMEOUT_EN, SEI_DAT_5, SEI_DAT_3, 32);  /* ST */
    ...
    sample_config.pos_data_use_rx = true;
    sample_config.rev_data_use_rx = true;
    #else
...
void isr_sei(void)
{
    ...
    if (sei_get_irq_status(BOARD_SEI, BOARD_SEI_CTRL, sei_irq_latch0_event)) {
        sei_clear_irq_flag(BOARD_SEI, BOARD_SEI_CTRL, sei_irq_latch0_event);
            ...
            //////////////////////////////////////////////////
         #else
            uint32_t rdc_theta = rdcObj.rdc_theta;///电角度（一对极，电角度=机械角度）
            sei_set_sample_pos_override_value(BOARD_SEI, BOARD_SEI_CTRL, rdc_theta);
        #endif
