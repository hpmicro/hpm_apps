...
void bldc_foc_al_speed(BLDC_CONTRL_SPD_PARA  *par)
{
    ...
    if (par->i_speedacq == par->num) {
        par->num = 0;
        par->o_speedout = HPM_MOTOR_MATH_DIV(par->speedthetalastn,par->i_speedlooptime_s);
...
void pmsm_foc_al_speed(MOTOR_PARA *par)
{
    ...
    float m_delta, m_encodercnt;
    m_delta = par->foc_para.speedcalpar.speedtheta - par->foc_para.speedcalpar.speedlasttheta;
#if (BOARD_PMSM0_ENCODER_TYPE)
    m_encodercnt = (float)BOARD_PMSM0_QEI_FOC_PHASE_COUNT_PER_REV;
#else
     m_encodercnt = (float)BOARD_PMSM0_SPI_FOC_PHASE_COUNT_PER_REV;
#endif
    if ((par->speedloop_para.target > 0) && (m_delta < -(m_encodercnt / 2)))
    {
        m_delta = m_delta + m_encodercnt;
    }
    else if((par->speedloop_para.target < 0) && (m_delta > (m_encodercnt / 2)))
    {
        m_delta = m_delta - m_encodercnt;
    }
    par->foc_para.speedcalpar.speedthetalastn += m_delta;
    par->foc_para.speedcalpar.speedlasttheta = par->foc_para.speedcalpar.speedtheta;
    par->foc_para.speedcalpar.num++;
    if (par->foc_para.speedcalpar.i_speedacq == par->foc_para.speedcalpar.num) {
        par->foc_para.speedcalpar.num = 0;
        par->foc_para.speedcalpar.o_speedout = HPM_MOTOR_MATH_DIV(par->foc_para.speedcalpar.speedthetalastn,par->foc_para.speedcalpar.i_speedlooptime_s);
        par->foc_para.speedcalpar.o_speedout_filter = par->foc_para.speedcalpar.o_speedout_filter + HPM_MOTOR_MATH_MUL(par->foc_para.speedcalpar.i_speedfilter,
            (par->foc_para.speedcalpar.o_speedout - par->foc_para.speedcalpar.o_speedout_filter));
        par->foc_para.speedcalpar.speedthetalastn = 0;
    }
}
/**********************************************
 * *void bldc_foc_sin_cos(HPM_MOTOR_MATH_TYPE angle, HPM_MOTOR_MATH_TYPE angle_precision,
                            HPM_MOTOR_MATH_TYPE *sin_angle, HPM_MOTOR_MATH_TYPE *cos_angle)
 * *sin and cos table
 * *@Input: electrical angle
 * *#Output：sin and cos value
 * ***************************************/
 ...
 * *subtract the sampling current after zero drift 
 * *@Input: None
 * *#Output：None
 * ***************************************/
void bldc_foc_current_cal(BLDC_CONTROL_CURRENT_PARA *par)
...
void bldc_foc_ctrl_dq_to_pwm(BLDC_CONTROL_FOC_PARA *par)
{
    ...
    bldc_foc_park(par->ialpha, par->ibeta,
                &par->currentdpipar.cur, &par->currentqpipar.cur,
                sin_angle, cos_angle);
#if defined (CONFIG_USE_FUNCTION) && (BOARD_PMSM0_CLC_3P3Z)
    CLC_control(par);
#else
