...
uint32_t foe_support_get_file_addr(uint16_t MBXMEM * name, uint16_t name_size)
{
    ...
    uint32_t addr = 0;
    uint8_t app_index = hpm_ota_get_nowrunning_appindex();
...
uint32_t foe_support_get_file_size(uint16_t MBXMEM * name, uint16_t name_size)
{
    ...
    uint8_t app_index = hpm_ota_get_nowrunning_appindex();
    hpm_app_header_t *app_header = hpm_ota_get_flash_header_info(app_index);
...
{
    int ret = hpm_platform_flash_init(); /* flash driver init */
    if (ret != 0) {
