...
static void set_reset_pin_level_pj089y2v5(uint8_t level)
{
    gpio_write_pin(HPM_GPIO0, GPIO_DO_GPIOE, 20, level);
}

static void set_video_router_pj089y2v5(void)
{
    pixelmux_mipi_dsi0_data_source_enable(pixelmux_mipi_dsi0_sel_lcdc0);
    pixelmux_config_tx_phy0_mode(pixelmux_tx_phy_mode_mipi);
}

static void set_video_router_pj089y2v5_2(void)
{
    pixelmux_mipi_dsi1_data_source_enable(pixelmux_mipi_dsi1_sel_lcdc1);
    pixelmux_config_tx_phy1_mode(pixelmux_tx_phy_mode_mipi);
}

void board_init_lcd_mipi_pj089y2v5(void)
{
    /* RESET */
    HPM_IOC->PAD[IOC_PAD_PE20].FUNC_CTL = IOC_PE20_FUNC_CTL_GPIO_E_20;
    gpio_set_pin_output(HPM_GPIO0, GPIO_DO_GPIOE, 20);
    ...
    hpm_panel_hw_interface_t hw_if = {0};
    hpm_panel_t *panel = hpm_panel_find_device_default(0);
    const hpm_panel_timing_t *timing = hpm_panel_get_timing(panel);
    uint32_t lcdc_pixel_clk_khz = board_lcdc_clock_init(clock_lcd0, timing->pixel_clock_khz);
    hw_if.set_reset_pin_level = set_reset_pin_level_pj089y2v5;
    hw_if.set_video_router = set_video_router_pj089y2v5;
    hw_if.lcdc_pixel_clk_khz = lcdc_pixel_clk_khz;
    hw_if.video.mipi.format = HPM_PANEL_MIPI_FORMAT_RGB888;
    hw_if.video.mipi.mipi_host_base = HPM_MIPI_DSI0;
    hw_if.video.mipi.mipi_phy_base = HPM_MIPI_DSI_PHY0;
    ...
    hpm_panel_t *panel_2 = hpm_panel_find_device_default(1);
    const hpm_panel_timing_t *timing_2 = hpm_panel_get_timing(panel_2);
    uint32_t lcdc_pixel_clk_khz_2 = board_lcdc_clock_init(clock_lcd1, timing_2->pixel_clock_khz);
    hw_if.set_reset_pin_level = set_reset_pin_level_pj089y2v5;
    hw_if.set_video_router = set_video_router_pj089y2v5_2;
    hw_if.lcdc_pixel_clk_khz = lcdc_pixel_clk_khz_2;
    hw_if.video.mipi.format = HPM_PANEL_MIPI_FORMAT_RGB888;
    hw_if.video.mipi.mipi_host_base = HPM_MIPI_DSI1;
    hw_if.video.mipi.mipi_phy_base = HPM_MIPI_DSI_PHY1;
    hpm_panel_register_interface(panel_2, &hw_if);

    printf("name: %s, lcdc_clk: %ukhz\n",
                        hpm_panel_get_name(panel_2),
                        lcdc_pixel_clk_khz_2);

    hpm_panel_reset(panel);
    hpm_panel_init(panel);
    hpm_panel_init(panel_2);
    
    hpm_panel_power_on(panel_2);
    hpm_panel_power_on(panel);
}
#endif

#if defined(CONFIG_PANEL_MIPI_ZE099DA) && CONFIG_PANEL_MIPI_ZE099DA
static void set_reset_pin_level_ze099da(uint8_t level)
{
    gpio_write_pin(HPM_GPIO0, GPIO_DO_GPIOE, 20, level);
}

static void set_video_router_ze099da(void)
{
    pixelmux_mipi_dsi0_data_source_enable(pixelmux_mipi_dsi0_sel_lcdc0);
    pixelmux_config_tx_phy0_mode(pixelmux_tx_phy_mode_mipi);
}

static void set_video_router_ze099da_2(void)
{
    pixelmux_mipi_dsi1_data_source_enable(pixelmux_mipi_dsi1_sel_lcdc1);
    pixelmux_config_tx_phy1_mode(pixelmux_tx_phy_mode_mipi);
}

void board_init_lcd_mipi_ze099da(void)
{
    /* RESET */
    HPM_IOC->PAD[IOC_PAD_PE20].FUNC_CTL = IOC_PE20_FUNC_CTL_GPIO_E_20;
    gpio_set_pin_output(HPM_GPIO0, GPIO_DO_GPIOE, 20);
    ...
    const hpm_panel_timing_t *timing = hpm_panel_get_timing(panel);
    uint32_t lcdc_pixel_clk_khz = board_lcdc_clock_init(clock_lcd0, timing->pixel_clock_khz);
    hw_if.set_reset_pin_level = set_reset_pin_level_ze099da;
    hw_if.set_video_router = set_video_router_ze099da;
    hw_if.lcdc_pixel_clk_khz = lcdc_pixel_clk_khz;
    hw_if.video.mipi.format = HPM_PANEL_MIPI_FORMAT_RGB888;
    hw_if.video.mipi.mipi_host_base = HPM_MIPI_DSI0;
    hw_if.video.mipi.mipi_phy_base = HPM_MIPI_DSI_PHY0;
    ...
    uint32_t lcdc_pixel_clk_khz_2 = board_lcdc_clock_init(clock_lcd1, timing_2->pixel_clock_khz);
    hw_if.set_reset_pin_level = set_reset_pin_level_ze099da;
    hw_if.set_video_router = set_video_router_ze099da_2;
    hw_if.lcdc_pixel_clk_khz = lcdc_pixel_clk_khz_2;
    hw_if.video.mipi.format = HPM_PANEL_MIPI_FORMAT_RGB888;
    hw_if.video.mipi.mipi_host_base = HPM_MIPI_DSI1;
    hw_if.video.mipi.mipi_phy_base = HPM_MIPI_DSI_PHY1;
...
uint32_t board_lcdc_clock_init(clock_name_t clock_name, uint32_t pixel_clk_khz)
{
    ...
    uint32_t freq_khz = clock_get_frequency(clk_pll1clk0) / 1000;
    uint32_t div = (freq_khz + pixel_clk_khz / 2) / pixel_clk_khz;
    clock_set_source_divider(clock_name, clk_src_pll1_clk0, div);
...
#if defined(CONFIG_PANEL_MIPI_PJ089Y2V5) && CONFIG_PANEL_MIPI_PJ089Y2V5
    board_init_lcd_mipi_pj089y2v5();
#endif

#if defined(CONFIG_PANEL_MIPI_ZE099DA) && CONFIG_PANEL_MIPI_ZE099DA
    board_init_lcd_mipi_ze099da();
#endif
}
...
void board_panel_para_to_lcdc(lcdc_config_t *config, uint8_t index)
{
    ...
    hpm_panel_t *panel = hpm_panel_find_device_default(index);
