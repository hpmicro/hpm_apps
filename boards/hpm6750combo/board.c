...
void init_sdxc_pwr_pin(SDXC_Type *ptr, bool as_gpio)
{
    if (ptr == HPM_SDXC1) {
        if (as_gpio) {
            ...
            HPM_IOC->PAD[IOC_PAD_PD10].PAD_CTL = IOC_PAD_PAD_CTL_DS_SET(1) | IOC_PAD_PAD_CTL_PE_SET(1) | 0x08 | IOC_PAD_PAD_CTL_PS_SET(1);
            gpio_set_pin_output_with_initial(HPM_GPIO0, GPIO_GET_PORT_INDEX(IOC_PAD_PD10), GPIO_GET_PIN_INDEX(IOC_PAD_PD10), 1);
        }
    }
}
...
void init_sdxc_vsel_pin(SDXC_Type *ptr, bool as_gpio)
{
    if (ptr == HPM_SDXC1) {
        if (as_gpio) {
            ...
            gpio_set_pin_output_with_initial(HPM_GPIO0, GPIO_GET_PORT_INDEX(IOC_PAD_PD13), GPIO_GET_PIN_INDEX(IOC_PAD_PD13), 1);
        }
    }
}
