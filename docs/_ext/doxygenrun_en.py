...
def doxygen_run(base_path, doxygen_path, app_name):
    doxygen_conf = os.path.join(doxygen_path, r'Doxyfile_en')
    output_path = os.path.join(str(HPM_APP_BASE / "docs" / "en" / "_static" / base_path), app_name)
                    ...
                    for root, dirs, files in os.walk(third_last_dir_path + '/doc'):
                        for file in files:
                            ...
                            if file.endswith(".png"):
                                ...
                                new_content.append('                         $(HPM_APP_BASE)/docs/en/' + base_path + '/' + dir_name + '/doc/api/assets/' + file_name + ' \\\n')
                            elif file.endswith(".pdf"):
                                file_name = os.path.basename(file_path)
                                new_content.append('                         $(HPM_APP_BASE)/docs/en/' + base_path + '/' + dir_name + '/doc/' + file_name + ' \\\n')
