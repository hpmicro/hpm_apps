...
def doxygen_auto_build(app: Sphinx) -> None:
    ...
    items = os.listdir(app.config.doxyrunner_outdir)
    for item in items:
        file_path = os.path.join(app.config.doxyrunner_outdir, item)
        if os.path.isdir(file_path):
            outdir = Path(app.config.doxyrunner_outdir) / Path(item) / Path(app.config.doxyrunner_outsubdir)
            print("------outdir:", outdir)
...
def doxygen_build(app: Sphinx) -> None:
    ...
    logger.info("Preparing Doxyfile...")
    print("````````doxyrunner_fmt_pattern:", app.config.doxyrunner_fmt_pattern);
    doxyfile = process_doxyfile(
        app.config.doxyrunner_doxyfile,
        tmp_outdir,
        app.config.doxyrunner_silent,
    ...
    logger.info("Running Doxygen...")
    # logger.info("Running: doxygen:", app.config.doxyrunner_doxygen, "\r\n--------\r\nsilent:", app.config.doxyrunner_silent)
    # logger.info("hpm_appbase:", app.config.HPM_APP_BASE)
    run_doxygen(
        app.config.doxyrunner_doxygen,
        doxyfile,
        app.config.doxyrunner_silent,
